...
def main(args):
    device = args.device
    data = torch.from_numpy(np.load(args.file_path)).float().to(device)
    T, D = data.shape
    ...
    model.eval()
    # log_weights = posterior_predictive_check(model, data.to(device), S=300, device=device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n_iter", "--n_iter", default=2, type=int)
    ...
    parser.add_argument("-file", "--file-path", default="hp_files/tc043_DIV14.npy", type=str)
    ...
    output_path = f"results/hp02_reconerror/output_{file_name}_K{args.number_feature}_seed{args.seed}_lambda{args.lambda_h}.pth"
    # output_path = f"results/newdata/output_{file_name}_K{args.number_feature}_seed{args.seed}.pth"

    torch.save(results, output_path)

    # Save the model's state dictionary

    model_path = f"results/hp02_reconerror/model_{file_name}_K{args.number_feature}_seed{args.seed}_lambda{args.lambda_h}.pth"
    # model_path = f"results/newdata/model_{file_name}_K{args.number_feature}_seed{args.seed}.pth"
